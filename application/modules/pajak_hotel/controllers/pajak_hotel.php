...
class Pajak_hotel extends Base_Controller {
  ...
  {
    $data['breadcrumbs'] = 'Daftar Pajak Hotel (Self Assessment)';
    $data['title'] = PRODUCT.' - '.$data['breadcrumbs'];
    $data['modul'] = 'pajak_hotel';
    $data['link_daftar'] = '/get_daftar_sa';
    $data['link_form'] = '/form_sa';
    $data['main_content'] = 'pajak_hotel_view';
    $data['tipe'] = 'sa';
  ...
  {
    $data['breadcrumbs'] = 'Daftar Pajak Hotel (Official Assessment)';
    $data['title'] = PRODUCT.' - '.$data['breadcrumbs'];
    $data['modul'] = 'pajak_hotel';
  ...
  function get_tarif_hotel($jumlah)
  {
    switch($jumlah)
    {
    ...
    $result = $this->data_model->get_tarif_hotel_by_kode($kode);
    if (count($result) > 0)
    {
  ...
  public function form_oa($id=0)
  {
    ...
    $data['form'] = '/form_oa';
    $data['header'] = 'Pajak Hotel (Official Assessment)';
    ...
    $data['main_content']='pajak_hotel_form';
    $this->load->view('layout/template',$data);
  }
  ...
  public function form_sa($id=0)
  {
    ...
    $data['form'] = '/form_sa';
    $data['header'] = 'Pajak Hotel (Self Assessment)';
    $data['akses'] = $this->access;
    if ($id!==0)
    {
      $data['data'] = $this->data_model->get_data_by_id($id);
  ...
  public function proses()
  {
    ...
    if ($this->form_validation->run() == TRUE){
      ...
      $tarif_hotel = $this->input->post('tarif_hotel');
      ...
      else if($tarif_hotel === '0')
      {
        $response->isSuccess = FALSE;
        $response->message = 'Tarif hotel untuk jumlah tersebut belum ada';
      }
      else
      {
        ...
        if (!$success)
        {
          ...
          $response->sql = $this->db->queries;
          $response->tarif_hotel = $tarif_hotel;
        }
        else
        {
          $response->isSuccess = FALSE;
  ...
  public function hapus()
  {
    ...
    if ($result) {
      ...
      $response->isSuccess = TRUE;
      $response->message = 'Data Pajak Hotel (Self Assessment) telah dihapus.';
    }
    else{
