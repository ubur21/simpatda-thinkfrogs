jQuery(document).ready(function(){
	...
	/*jQuery("#rekeningTable").jqGrid({
		url: root+'/master/rekening/get_rekening/gol.c',
		datatype: 'json',
		mtype: 'POST',
...
jQuery(document).ready(function(){
	...
	jQuery("#npwprdTable").jqGrid({
		url: root+'/pendaftaran/seeknpw/PAJAK/06',
		datatype: 'json',
		mtype: 'POST',
	...
	jQuery("#dataTable").jqGrid({
		...
		colModel :[
			...
			{name:'kode_rekening',index:'kode_rekening',width:225,editable:true,editrules: {edithidden:true},edittype:'select', 
				editoptions: {
					dataUrl:root+'/master/rekening/gridselect/gol.c', 
					style:'width:200px;',
					dataEvents:[
						{ type: 'change',
						  fn:function(e){
...
jQuery(document).ready(function(){ 
	...
	jQuery('#trigger_npw').click(function(){
		...
		jQuery("#npwprdTable").setGridParam({url:root+'/pendaftaran/seeknpw/'+x+'/06', datatype:'json'}).trigger("reloadGrid");
