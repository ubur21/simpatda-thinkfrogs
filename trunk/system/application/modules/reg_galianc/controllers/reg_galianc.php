...
class reg_galianc extends Controller
{	
	var $error_msg;
	var $result_msg;
	
	function reg_galianc()
	{
		parent::Controller();
		$this->error_msg=''; $this->result_msg='';
		//$this->load->library('firephp');
		//$this->load->library('jqgridcss','','css');
		$this->load->model('reg_galianc_model','data_model');
	...
	function daftar()
	{
		...
		{				
			...
			$owhere['where'] = "a.jenis_pendataan='GALIANC' ";
	...
	function grid_form()
	{
		...
		if(isset($id) && $id!=''){
			...
			$limit = jqgrid_set_limit('pendataan_galianc a',$owhere);
			...
			$query  = 'select a.*, b.kode_rekening,b.nama_rekening from pendataan_galianc a join rekening_kode b on b.id=a.id_rekening where a.pendataan_id='.$this->db->escape($id);
			...
			{  $i=0;
			   ...
			   {
					$response->rows[$i]['id']=$row->GALIANC_ID;
