...
class reg_listrik extends Controller
{	
	var $error_msg;
	var $result_msg;
	
	function reg_listrik()
	{
		parent::Controller();
		$this->error_msg=''; $this->result_msg='';
		//$this->load->library('firephp');
		//$this->load->library('jqgridcss','','css');
		$this->load->model('reg_listrik_model','data_model');
	...
	function daftar()
	{
		...
		{				
			...
			$owhere['where'] = "a.jenis_pendataan='LISTRIK'";
	...
	{		
		...
		$arr['kode_rekening']  = $this->input->post('kode_rekening');
		$arr['idrekening']     = $this->input->post('idrekening');
		$arr['kva']		   	   = str_replace(',','',$this->input->post('kva'));
		$arr['diskon']	   	   = str_replace(',','',$this->input->post('diskon'));
		$arr['jam']	   	   	   = str_replace(',','',$this->input->post('jam'));
