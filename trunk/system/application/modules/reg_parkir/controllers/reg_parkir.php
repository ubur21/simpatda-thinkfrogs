...
class reg_parkir extends Controller
{	
	var $error_msg;
	var $result_msg;
	...
	function reg_parkir()
	{
		...
		$this->load->model('reg_parkir_model','data_model');
	...
	function daftar()
	{
		...
		{				
			...
			$owhere['where'] = "a.jenis_pendataan='PARKIR' ";
	...
	function grid_form()
	{
		...
		if(isset($id) && $id!=''){
			...
			$limit = jqgrid_set_limit('pendataan_pparkir a',$owhere);
			...
			$query  = 'select a.*, b.kode_rekening,b.nama_rekening from pendataan_pparkir a join rekening_kode b on b.id=a.id_rekening where a.pendataan_id='.$this->db->escape($id);
			...
			{  $i=0;
			   ...
			   {
					$response->rows[$i]['id']=$row->PPARKIR_ID;
