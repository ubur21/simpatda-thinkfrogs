...
jQuery(document).ready(function(){
	...
	jQuery("#npwprdTable").jqGrid({
		url: root+'/pendaftaran/seeknpw/PAJAK/01',
		datatype: 'json',
		mtype: 'POST',
...
jQuery(document).ready(function(){ 
	...
	jQuery("#daftarTable").jqGrid({
		...
		'krekening','nrekening','irekening','persen_tarif','dpengenaan','nominal','hotel_nama','hotel_alamat'],
		colModel :[
			{name:'id',index:'id',width:20,search:false},
			...
			{name:'hotel_nama',index:'hotel_nama',hidden:true,search:false},
			{name:'hotel_alamat',index:'hotel_alamat',hidden:true,search:false},
		],
        ...
        ondblClickRow: function(id){ 
			...
			jQuery.get(root+'penetapan_pr/ed_spt/'+id+'_'+data.jenis_pungutan,function(dt){
				...
				if(dt<1)
				{
					...
					jQuery('#nama_hotel').val(data.hotel_nama);
					jQuery('#alamat_hotel').val(data.hotel_alamat);
	...
	jQuery('#trigger_npw').click(function(){
		...
		jQuery("#npwprdTable").setGridParam({url:root+'/pendaftaran/seeknpw/'+x+'/01', datatype:'json'}).trigger("reloadGrid");
