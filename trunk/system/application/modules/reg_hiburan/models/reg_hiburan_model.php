...
class reg_hiburan_model extends Model {
	...
	function reg_hiburan_model()
	{
		parent::Model();
		$this->_table    = 'pendataan_spt';
		$this->_table_pk = 'pendataan_id';	
		
		$this->_table_rincian    = 'pendataan_phiburan';
		$this->_table_rincian_pk = 'phiburan_id';
	...
	function fill_rincian_data($arr=array())
	{
		if($arr['ID']=='')
		{
			$this->data_rincian = array(
				'pendataan_id'=>$this->input->post('idmasters')
	...
	function get_rekening($arr='')
	{
		...
		$this->db->from('rekening_kode a');
		//$this->db->where("lower(a.nama_rekening) like '%walet%' ");
		$this->db->where("kode_rekening like '4.1.1.03.%' ");
