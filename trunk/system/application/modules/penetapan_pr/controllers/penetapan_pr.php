<?php

class penetapan_pr extends Controller
{	
	var $error_msg;
	var $result_msg;
	
	function penetapan_pr()
	{
		parent::Controller();
		$this->error_msg=''; $this->result_msg='';
		//$this->load->library('firephp');
		//$this->load->library('jqgridcss','','css');
		$this->load->model('penetapan_pr_model','data_model');
		
		$this->external->set(array(
            'css' => base_url().'assets/css/jquery.autocomplete.css',
            'js' => array(base_url().'assets/script/jquery/jquery.autocomplete.min.js'
						  ,base_url().'assets/script/jquery/jquery.metadata.js'
						  ,base_url().'assets/script/jquery/jquery.maskedinput.js'
						  ,base_url().'assets/script/jquery/jquery.validate.min.js'
						  ,base_url().'assets/script/jquery/jquery.formatCurrency-1.4.0.js'
					),
        ));
		
	}
		
	function index()
	{		
		if(b_logged()){
		
			$tgl = date('d/m/Y');
			
			$jatem_bayar = $this->db->query('select jatem_bayar from ref_jatuh_tempo')->row()->JATEM_BAYAR;
			
			$tgl_tempo = getExpired($tgl,$jatem_bayar);
			
			$arr = array('is_save'=>1,'is_cancel'=>1);
			$data['tgl_penetapan']= $tgl;
			$data['tgl_setor']	  = $tgl_tempo;
			$data['mod']		  = 'penetapan_pr';
			$data['button_form']  = $this->load->view('element/button',$arr,true);
			$data['title_form']   = get_name_menu($this->uri->segment(1));
			$data['form'] 		  = $this->load->view('form',$data,true);
			$data['main_content'] = $this->load->view('daftar',$data,true);
								
			$this->load->vars($data);
			
		}else{
			$data['main_content'] = $this->load->view('login/login','',true);
		}
		
		$this->load->view($this->config->item('layout_dir').'/template', $data);

	}
	
	function js()
	{
		$this->load->view('js/script.js');
	}
	
	function get_expired()
	{
		
		$jatem_bayar = $this->db->query('select jatem_bayar from ref_jatuh_tempo')->row()->JATEM_BAYAR;
		
		$tgl_tempo = getExpired($_REQUEST['tanggal'],$jatem_bayar);
		
		echo $tgl_tempo;

		
	}
	
	function daftar()
	{		
		
		if(!isset($_POST['oper']))
		{				
			$id = $this->uri->segment(3);
			
			//$owhere['where'] = "a.jenis_pendataan='RETRIBUSI'";
			
			$this->db->join('penetapan_pr_content b','a.penetapan_pr_id=b.penetapan_pr_id','left');
			$this->db->join('pendataan_spt c','c.pendataan_id=b.pendataan_id','left');
			$limit = jqgrid_set_limit('penetapan_pr a');
			
			//$limit['where'] = $owhere['where'];
			
			$response->page = $limit['page']; 
			$response->total = $limit['total_pages'];
			$response->records = $limit['records'];		
			
			$result = $this->data_model->get_data($limit)->result_array();
																
			for($i=0; $i<count($result); $i++){
									
				$response->rows[$i]['id']=$result[$i]['PENETAPAN_PR_ID'];
																				
				$response->rows[$i]['cell']=array($result[$i]['PENETAPAN_PR_ID'],
												format_date($result[$i]['TGL_PENETAPAN']),
												sprintf('%05d',$result[$i]['NO_PENETAPAN']),
												number_format($result[$i]['NOMINAL_PENETAPAN'],2,'.',','),
												sprintf('%05d',$result[$i]['PENDATAAN_NO']),
												format_date($result[$i]['TGL_SETOR']),
												format_date($result[$i]['TGL_ENTRY']),
												
											);
			}
			echo json_encode($response);
			
		}else{		
			$id = $this->input->post('id');
			if($_POST['oper'] === 'add'){
			
				$this->insert();
				
			}else if($_POST['oper'] === 'edit'){
			
				$this->update($id);
				
			}else if($_POST['oper'] === 'del'){
			
				if(!$this->data_model->isAbleDelete())
				{
					$this->db->trans_start();				
					if($this->data_model->delete_rincian_data($id)){
						if($this->data_model->delete_data($id))
						{
							$this->db->trans_complete();
						}
					}
				}
			}
		}	
	}
	
	function pilih_kohir()
	{
		$id = $this->uri->segment(3);
			
		$this->db->join('penetapan_pr_content b','a.penetapan_pr_id=b.penetapan_pr_id','left');
		$this->db->join('penerimaan_pr c','c.PENETAPAN_PR_ID=a.PENETAPAN_PR_ID','left');
		
		$owhere['where'] = "c.PENETAPAN_PR_ID is null";
		
		$limit = jqgrid_set_limit('penetapan_pr a',$owhere);
		
		$limit['where'] = $owhere['where'];
		
		$response->page = $limit['page']; 
		$response->total = $limit['total_pages'];
		$response->records = $limit['records'];		
		
		$result = $this->data_model->pilih_kohir($limit)->result_array();
							
		for($i=0; $i<count($result); $i++){
								
			$response->rows[$i]['id']=$result[$i]['PENETAPAN_PR_ID'];
																			
			$response->rows[$i]['cell']=array($result[$i]['PENETAPAN_PR_ID'],
											$result[$i]['NO_PENETAPAN'],
											format_date($result[$i]['TGL_PENETAPAN']),
											format_date($result[$i]['TGL_SETOR']),
											number_format($result[$i]['NOMINAL'],2,'.',',')
										);
		}
		echo json_encode($response);	
	
	}	
	
	function sptprd()
	{		
		$owhere['where'] = "b.pendataan_id is null and a.JENIS_PUNGUTAN='OFFICE' ";
		
		$this->db->join('penetapan_pr_content b','b.pendataan_id=a.pendataan_id','left');
		
		$limit = jqgrid_set_limit('v_pendataan_spt a',$owhere);
		
		$limit['where'] = $owhere['where'];
						
		$response->page = $limit['page']; 
		$response->total = $limit['total_pages'];
		$response->records = $limit['records'];
				
		$result = $this->data_model->get_sptprd($limit)->result_array();
				
		for($i=0; $i<count($result); $i++){
		
			$sistem_pungutan = $result[$i]['JENIS_PUNGUTAN']=='OFFICE' ? 'Office Assesment' : 'Selft Assesment';
					
			$response->rows[$i]['id']=$result[$i]['ID'];
			// data berikut harus sesuai dengan kolom-kolom yang ingin ditampilkan di view (js)
			$response->rows[$i]['cell']=array($result[$i]['ID'],
											$result[$i]['PENDATAAN_NO'],												
											$result[$i]['TGL_ENTRY'],
											$result[$i]['NPWP'],
											$result[$i]['PEMOHON'],
											$result[$i]['JENIS_PENDATAAN'],
											$sistem_pungutan,
											$result[$i]['JENIS_PENDAFTARAN'],
											$result[$i]['SPT_NO'],
											$result[$i]['TGL_SPT'],
											$result[$i]['NOMINAL']
										);
		}
		echo json_encode($response);	
	}
	
	function getno()
	{
		$nomor = $this->data_model->get_next_no();
		echo sprintf('%05d',$nomor);
	}	
	
	function submit()
	{			
		$this->db->trans_start();
		
		if(isset($_REQUEST['action']) && $_REQUEST['action']=='edit')
		{

			
		}else{
			
			foreach($_REQUEST['idrows'] as $idx=>$value)
			{
				$arr['ID']    = $this->data_model->setID();
				$arr['nomor'] = $this->data_model->get_next_no();
				
				$header_total = 0;
								
				//$arr['total'] = str_replace(',','',$this->input->post('dasar_pengenaan'));
				
				$this->data_model->fill_data($arr);
				
				$hsl = $this->data_model->insert_data();
				
				/*
								$this->data_mode;->data = array(
											'penetapan_pr_id'=>$arr['ID']
											,'no_penetapan'=>$arr['nomor']
											,'tgl_penetapan'=>prepare_date($this->input->post('tgl_penetapan'))
											,'thn_penetapan'=>date('Y',strtotime($this->input->post('tgl_penetapan')))
											,'tgl_setor'=>prepare_date($this->input->post('tgl_setor'))
											,'memo'=>$this->input->post('memo')
											,'nominal_penetapan'=>0
											,'LOGS'=>'NOW'
											,'user_id'=>$this->session->userdata('SESS_USER_ID')
										);
				*/
				if($hsl)
				{
					$arr['pendataan_id'] = $value;
					
					$rs = $this->db->query('select nominal,jenis_pendataan,jenis_pungutan from pendataan_spt where pendataan_id='.$this->db->escape($value))->row();
					
					$this->db->query('update penetapan_pr set nominal_penetapan='.$this->db->escape($rs->NOMINAL).' where penetapan_pr_id='.$this->db->escape($arr['ID']));

					$this->data_model->data_rincian = array(
										'penetapan_pr_id'=>$arr['ID']
										,'pendataan_id'=>$arr['pendataan_id']
										,'nominal'=>$rs->NOMINAL
										,'jenis_pendataan'=>$rs->JENIS_PENDATAAN
										,'jenis_pungutan'=>$rs->JENIS_PUNGUTAN
									);
									
					$hsl = $this->data_model->insert_rincian_data();
					
					$result = $this->db->query('select pendataan_content_id,id_rekening,persen_tarif,dasar_tarif,dasar_pengenaan,nominal from v_pendataan_content where pendataan_id='.$this->db->escape($value));
					
					if($result->num_rows()>0)
					{
						foreach($result->result() as $row)
						{
							$this->data_model->data_detail = array(
								'penetapan_pr_id'=>$arr['ID']
								,'pendataan_id'=>$arr['pendataan_id']
								,'pendataan_content_id'=>$row->PENDATAAN_CONTENT_ID
								,'id_rekening'=>$row->ID_REKENING
								,'persen_tarif'=>$row->PERSEN_TARIF
								,'dasar_tarif'=>$row->DASAR_TARIF
								,'dasar_pengenaan'=>$row->DASAR_PENGENAAN
								,'nominal'=>$row->NOMINAL
								/*,'kva'=>$arr['kva']
								,'diskon'=>$arr['diskon']
								,'jam'=>$arr['jam']
								,'tgl_jatuh_tempo'=>$arr['tgl_jatuh_tempo']*/
							);
							
							//$this->data_model->fill_rincian_data($arr);
							
							$hsl = $this->data_model->insert_detail_data();
						}
					}
				}
								
				if($hsl) $this->result_msg='<li>:: Data telah tersimpan ::</li>';
				else $this->result_msg='<li>:: Error tersimpan data ::</li>';
			}
			
			echo $this->result_msg;			
		}
		$this->db->trans_complete();
		
	}
	
	function ed_spt()
	{
		$id = $this->uri->segment(3);
		
		$arr = explode('_',$id);
		
		if($arr[1]=='OFFICE'){
			
			$cek = $this->db->query('select count(*) from penetapan_pr_content where pendataan_id='.$this->db->escape($arr[0]))->row()->COUNT;
			
		}else{		
		
			$cek = $this->db->query('select count(*) from penerimaan_pr where pendataan_id='.$this->db->escape($arr[0]))->row()->COUNT;
		
		}

		echo $cek;
	}
	
	function form()
	{		
		$this->submit();		
	}

}

?>