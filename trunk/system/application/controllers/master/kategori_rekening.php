...
class kategori_rekening extends Controller
{	
	var $error_msg;
	
	function kategori_rekening()
	{
		parent::Controller();
		$this->error_msg='';
		$this->load->library('firephp');
		$this->load->model('kategori_rekening_model','data_model');
	}
	
	function index()
	{		
		if(b_logged()){
			$data['main_content'] = $this->load->view('master/daftar_kategori_rekening','',true);
		}else{
		...
		{				
			...
			for($i=0; $i<count($result); $i++){
				...
				$response->rows[$i]['cell']=array($result[$i]['ID'],
												$result[$i]['TIPE_KATEGORI'],
												$result[$i]['NAMA_KATEGORI'],
												$result[$i]['SALDO_NORMAL']
											);
			}
