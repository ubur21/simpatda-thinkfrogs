...
class Kedudukan extends Controller {

	function Kedudukan()
	{
		parent::Controller();
		$this->load->model('kedudukan_model', 'data_model');
	}
	
	function index()
	{
		if(!isset($_POST['oper'])){
			$data['title'] = "SIPTU - Kedudukan Hukum";
			$data['main_content'] = 'master/kedudukan';
    ...
    function get_daftar()
    {
        ...
        for($i=0; $i<count($result); $i++){
            $response->rows[$i]['id']=$result[$i]['ID_KEDUDUKAN'];
            // data berikut harus sesuai dengan kolom-kolom yang ingin ditampilkan di view (js)
            $response->rows[$i]['cell']=array($result[$i]['ID_KEDUDUKAN'],
											$result[$i]['NAMA_KEDUDUKAN']
                                        );
        }
	...
	function insert()
	{
		...
		{
			$error_msg = 'Nama Kedudukan telah digunakan';
		}
		//Insert Data
	...
	function delete()
	{
		...
		$data = $this->data_model->get_data_by_id($id);
		{
			...
			{
				$error_msg = 'Terjadi kesalahan dalam menghapus data Kedudukan '.$data['NAMA_KEDUDUKAN'].'. Harap coba lagi.';
			}
		}
		echo json_encode(array('errors'=> $error_msg));
	}
...
/* End of file kedudukan.php */
/* Location: ./application/controllers/kedudukan.php */
