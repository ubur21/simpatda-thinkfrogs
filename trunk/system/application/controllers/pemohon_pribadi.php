...
class pemohon_pribadi extends Controller
{	
	var $error_msg;
	
	function pemohon_pribadi()
	{
		parent::Controller();
		$this->error_msg='';
	...
	{		
		if(b_logged()){
			...
			$data['desa'] = $desa;
			$data['main_content'] = $this->load->view('pendaftaran/daftar_pemohon_pribadi',$data,true);
	...
	function getinfo()
	{
		...
		if($result)
		{
			...
			$arr['TANGGAL_LAHIR'] = format_date($result['TANGGAL_LAHIR']);
			$arr['ada'] = 1;
		}else{
			$arr['ada'] = 0;
		}
		...
		{				
			...
			for($i=0; $i<count($result); $i++){
				...
				$response->rows[$i]['cell']=array($result[$i]['PRIBADI_ID'],
												...
												$result[$i]['PEKERJAAN'],
												$this->db->getBlob($result[$i]['ALAMAT']),
												$result[$i]['RT'],
												$result[$i]['RW'],
												$result[$i]['KODEPOS'],
												$result[$i]['ID_DESA'],
