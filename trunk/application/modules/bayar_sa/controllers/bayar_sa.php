...
class Bayar_sa extends Base_Controller {

  public function __construct()
  {
    parent::__construct();
		$this->load->model('bayarsa_model','data_model');
  }
  ...
  {
    $data['breadcrumbs'] = 'Daftar Pembayaran Self Assessment';
    $data['title'] = PRODUCT.' - '.$data['breadcrumbs'];
    $data['modul'] = 'bayar_sa';
    $data['main_content'] = 'bayarsa_view';
  ...
  public function get_daftar()
  {
	...
	function getsptlngkp()
	{
		$id_spt = $this->input->post('id_spt') ? $this->input->post('id_spt') : 0;
		$idpjk = $this->input->post('idpjk') ? $this->input->post('idpjk') : 0;
		$result = $this->data_model->getsptlngkp($id_spt, $idpjk);
		$response = (object) NULL;
		$response->sql = $this->db->queries;
		$response->len = count($result);
		//print_r($result);
  ...
  public function form($id=0)
  {
    ...
    $data['main_content']='bayarsa_form';
    $this->load->view('layout/template',$data);
  }
