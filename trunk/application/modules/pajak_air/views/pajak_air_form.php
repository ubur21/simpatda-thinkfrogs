...
<form id="frm" method="post" action="<?php echo base_url(); ?>pajak_air/<?php echo $link_proses;?>">
  <fieldset>
  ...
  <fieldset>
    ...
    <div class="controls-row" >
      <div class="control-group pull-left" >
        <label class="control-label" for="jumlah">Jumlah (m3)</label>
        <input type="text" class="span3 currency" id="jumlah" data-bind="numeralvalue: jumlah, valueUpdate: 'keyup'" />
      </div>
...
$(document).ready(function() {
  ...
  var ModelPendaftaran = function (){
    ...
    self.jumlah = ko.observable('<?php echo isset($data['JUMLAH']) ? $data['JUMLAH'] : 0 ?>');
   ...
   self.isEdit = ko.computed(function(){
    ...
    self.jumlah.subscribe(function(newValue){
      $.getJSON(root+modul+'/get_tarif_air/'+self.jumlah(), function(data){
        if (data === 0)
        {
          alert('Tarif air untuk jumlah tersebut belum ada');
        }
        self.tarif_air(data);
        //hitung = self.jumlah() * self.tarif1() * (self.tarif2() / 100) * self.tarif_air();
		hitung = self.jumlah() * self.tarif1() * (self.tarif2() / 100);
        self.jml(hitung);
      });
    })
  ...
  App.formValidation = function(){
    var errmsg = [];
    var tarif_air = App.tarif_air();
    if (tarif_air === 0){
      errmsg.push('Tarif air untuk jumlah tersebut belum ada.');
  ...
  App.pilih_rekening = function(){
    ...
    var option = {multi:0, mode:'pendataan_air'};
    Dialog.pilihRekening(option, function(obj, select){
      ...
      //var hitung = App.jumlah() * App.tarif1() * (App.tarif2() / 100) * App.tarif_air();
      var hitung = App.jumlah() * App.tarif1() * (App.tarif2() / 100);
      App.jml(hitung);
    });
  }
