...
class Jurnal_model extends CI_Model {
	...
	function insert_data()
	{
		...
		$newid = $this->db->query('select max(ID) as ID from MST_JURNAL')->row_array();
		$this->db->trans_complete();
		return $newid['ID'];
	}
...
*/	
	function check_isi()
	{
		$AKUN = $this->input->post('nama');$AKUN=($AKUN)?$AKUN:null;
		
		$this->db->trans_start();
		if ($AKUN != null){
			$query = "SELECT * FROM MST_JURNAL WHERE NAMA='$AKUN'";
	...
	function check_isi2($id) // cek NIP sebelum update
	{
		...
		$this->db->trans_start();
		if ($AKUN != null){
			$query = "SELECT * FROM MST_JURNAL WHERE KODE='$AKUN' and ID!='$id'";
			$result = $this->db->query($query);
			$result = count($result->result_array());
