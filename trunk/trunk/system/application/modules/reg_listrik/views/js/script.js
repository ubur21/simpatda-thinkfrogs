...
jQuery(document).ready(function(){
	...
	jQuery("#npwprdTable").jqGrid({
		url: root+'/pendaftaran/seeknpw/PAJAK/05',
		datatype: 'json',
		mtype: 'POST',
...
function hitungPajak(){
	...
	idasar = document.getElementById('dasar_tarif').value!='' ? removeCommas(document.getElementById('dasar_tarif').value) : 1;
	...
	jQuery('#dasar_pengenaan').val(nominal).formatCurrency({decimalSymbol:'.',digitGroupSymbol:',',roundToDecimalPlace:'2',symbol:''});
...
jQuery(document).ready(function(){ 
	...
	jQuery("#daftarTable").jqGrid({
		...
		'krekening','nrekening','irekening','kva','diskon','jam','persen_tarif','dtarif','dpengenaan','nominal'],
		colModel :[
			{name:'id',index:'id',width:20,search:false},
			...
			{name:'kva',index:'kva',hidden:true,search:false},
			{name:'diskon',index:'diskon',hidden:true,search:false},
			{name:'jam',index:'jam',hidden:true,search:false},
			{name:'persen_tarif',index:'persen_tarif',hidden:true,search:false},
        ...
        ondblClickRow: function(id){ 
			...
			jQuery.get(root+'penetapan_pr/ed_spt/'+id+'_'+data.jenis_pungutan,function(dt){
				...
				if(dt<1)
				{
					...
					jQuery('#kva').val(data.kva).formatCurrency({decimalSymbol:'.',digitGroupSymbol:',',roundToDecimalPlace:'2',symbol:''});
					jQuery('#diskon').val(data.diskon).formatCurrency({decimalSymbol:'.',digitGroupSymbol:',',roundToDecimalPlace:'2',symbol:''});
					jQuery('#jam').val(data.jam).formatCurrency({decimalSymbol:'.',digitGroupSymbol:',',roundToDecimalPlace:'2',symbol:''});
					jQuery('#persen_tarif').val(data.persen_tarif).formatCurrency({decimalSymbol:'.',digitGroupSymbol:',',roundToDecimalPlace:'2',symbol:''});
	...
	jQuery("#kva").keyup(function(){ jQuery(this).value=cnumber(this); });
	jQuery("#kva").keyup(function(){ jQuery(this).value=focus_uang(this); });
	jQuery("#kva").blur(function(){ jQuery(this).value=blur_uang(this); hitungPajak(); });
	
	jQuery("#jam").keyup(function(){ jQuery(this).value=cnumber(this); });
	jQuery("#jam").blur(function(){ jQuery(this).value=focus_uang(this); });
	jQuery("#jam").blur(function(){ jQuery(this).value=blur_uang(this); hitungPajak(); });
	
	jQuery("#tarif_dasar").keyup(function(){ jQuery(this).value=cnumber(this); });
	jQuery("#tarif_dasar").blur(function(){ jQuery(this).value=blur_uang(this); hitungPajak(); });
	
	jQuery("#diskon").keyup(function(){ jQuery(this).value=cnumber(this);hitungPajak(); });
	jQuery("#diskon").blur(function(){ jQuery(this).value=focus_uang(this); });
	jQuery("#diskon").blur(function(){ jQuery(this).value=blur_uang(this); });
	
	jQuery("#nominal").keyup(function(){ jQuery(this).value=cnumber(this); });
	jQuery("#nominal").keyup(function(){ jQuery(this).value=focus_uang(this); });
	jQuery("#nominal").blur(function(){ jQuery(this).value=blur_uang(this); });
	...
	jQuery('#trigger_npw').click(function(){
		...
		jQuery("#npwprdTable").setGridParam({url:root+'/pendaftaran/seeknpw/'+x+'/05', datatype:'json'}).trigger("reloadGrid");
