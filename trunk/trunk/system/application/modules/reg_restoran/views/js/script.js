...
jQuery(document).ready(function(){
	...
	jQuery("#npwprdTable").jqGrid({
		url: root+'/pendaftaran/seeknpw/PAJAK/02',
		datatype: 'json',
		mtype: 'POST',
...
jQuery(document).ready(function(){ 
	...
	jQuery("#daftarTable").jqGrid({
		...
		'krekening','nrekening','irekening','persen_tarif','dpengenaan','nominal','restoran_nama','restoran_nama'],
		colModel :[
			{name:'id',index:'id',width:20,search:false},
			...
			{name:'restoran_nama',index:'restoran_nama',hidden:true,search:false},
			{name:'restoran_alamat',index:'restoran_alamat',hidden:true,search:false},
		],
        ...
        ondblClickRow: function(id){ 
			...
			jQuery.get(root+'penetapan_pr/ed_spt/'+id+'_'+data.jenis_pungutan,function(dt){
				...
				if(dt<1)
				{
					...
					jQuery('#nama_restoran').val(data.restoran_nama);
					jQuery('#alamat_restoran').val(data.restoran_alamat);
	...
	jQuery('#trigger_npw').click(function(){
		...
		jQuery("#npwprdTable").setGridParam({url:root+'/pendaftaran/seeknpw/'+x+'/02', datatype:'json'}).trigger("reloadGrid");
