...
class reg_hiburan extends Controller
{	
	var $error_msg;
	var $result_msg;
	
	function reg_hiburan()
	{
		parent::Controller();
		$this->error_msg=''; $this->result_msg='';
		//$this->load->library('firephp');
		//$this->load->library('jqgridcss','','css');
		$this->load->model('reg_hiburan_model','data_model');
	...
	function daftar()
	{
		...
		{				
			...
			$owhere['where'] = "a.jenis_pendataan='HIBURAN' ";
	...
	function grid_form()
	{
		...
		if(isset($id) && $id!=''){
			...
			$query  = 'select a.*, b.kode_rekening,b.nama_rekening from pendataan_phiburan a left join rekening_kode b on b.id=a.id_rekening where a.pendataan_id='.$this->db->escape($id);
			...
			{  $i=0;
			   ...
			   {
					$response->rows[$i]['id']=$row->PHIBURAN_ID;
