...
jQuery(document).ready(function(){
	...
	jQuery("#npwprdTable").jqGrid({
		url: root+'/pendaftaran/seeknpw/PAJAK/07',
		datatype: 'json',
		mtype: 'POST',
...
jQuery(document).ready(function(){
	...
	jQuery("#dataTable").jqGrid({
		...
		datatype: 'json',
		mtype: 'POST',
		colNames:['id','Kode Rekening','Dasar Pengenaan','Persen Tarif','Pajak'],
		colModel :[
			{name:'id',index:'id',width:20,search:false},
			{name:'kode_rekening',index:'kode_rekening',width:225,editable:true,editrules: {edithidden:true},edittype:'select', 
				editoptions: {
					dataUrl:root+'/master/rekening/gridselect/parkir', 
					style:'width:200px;',
					dataEvents:[
						{ type: 'change',
						  fn:function(e){
							jQuery.ajax({url:root+'/master/rekening/detail_selectgrid/'+this.value,dataType:'json',
								success: function(json){
									jQuery('input#dasar_pengenaan').val(json.tarif);
...
jQuery(document).ready(function(){ 
	...
	function showResponse(responseText, statusText, xhr, form)
	{
		...
		jQuery('#dataTable').setGridParam({url:root+mod+'/grid_form'}).trigger('#reloadGrid');
		jQuery('#dataTable').setGridParam({editurl:root+mod+'/grid_form'}).trigger('#reloadGrid');
		v.resetForm(); 
	...
	jQuery('#trigger_npw').click(function(){
		...
		jQuery("#npwprdTable").setGridParam({url:root+'/pendaftaran/seeknpw/'+x+'/07', datatype:'json'}).trigger("reloadGrid");
