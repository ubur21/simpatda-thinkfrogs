...
class reg_wallet extends Controller
{	
	var $error_msg;
	var $result_msg;
	...
	function reg_wallet()
	{
		...
		$this->load->model('reg_wallet_model','data_model');
	...
	function daftar()
	{
		...
		{				
			...
			$owhere['where'] = "a.jenis_pendataan='WALLET' ";
	...
	function grid_form()
	{
		...
		if(isset($id) && $id!=''){
			...
			$query  = 'select a.*, b.kode_rekening,b.nama_rekening from pendataan_pwalet a left join rekening_kode b on b.id=a.id_rekening where a.pendataan_id='.$this->db->escape($id);
