...
class Pendidikan extends Controller {
	...
	{
		parent::Controller();
		$this->load->model('pendidikan_model', 'data_model');
	}
	
	function index()
	{
		if(!isset($_POST['oper'])){
			$data['title'] = "SIPTU - Pendidikan";
			$data['main_content'] = 'master/pendidikan';
	...
	function getselect()
	{
		...
		for($i=0; $i<count($row); $i++){
			echo "<option value='". $row[$i]->NAMA_PENDIDIKAN ."'>". $row[$i]->NAMA_PENDIDIKAN ."</option>";
		}
		echo "</select>";
	}
    ...
    function get_daftar()
    {
        ...
        for($i=0; $i<count($result); $i++){
            $response->rows[$i]['id']=$result[$i]['ID_PENDIDIKAN'];
            // data berikut harus sesuai dengan kolom-kolom yang ingin ditampilkan di view (js)
            $response->rows[$i]['cell']=array($result[$i]['ID_PENDIDIKAN'],
											$result[$i]['URUT'],
											$result[$i]['NAMA_PENDIDIKAN']
                                        );
        }
	...
	function insert()
	{
		...
		{
			$error_msg = 'Nama Pendidikan telah digunakan';
		}
		//Insert Data
	...
	function delete()
	{
		...
		$data = $this->data_model->get_data_by_id($id);
		{
			...
			{
				$error_msg = 'Terjadi kesalahan dalam menghapus data Pendidikan '.$data['NAMA_PENDIDIKAN'].'. Harap coba lagi.';
			}
		}
		echo json_encode(array('errors'=> $error_msg));
	}
