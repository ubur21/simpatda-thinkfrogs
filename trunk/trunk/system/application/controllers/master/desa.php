...
{	
	...
	{		
		if(b_logged()){
			...
			$row = $this->kecamatan->get_all_data();
			for($i=0; $i<count($row); $i++){
				$kecamatan.="'".$row[$i]->CAMAT_ID."':'".$row[$i]->CAMAT_NAMA." (".$row[$i]->CAMAT_KODE.")',";
			}
			$data['kecamatan'] = $kecamatan;
			
			$data['main_content'] = $this->load->view('master/daftar_desa','',true);
		}else{
	...
	function seekname()
	{
		...
		$query = $this->desa->seekname($nama);
		if($query->num_rows() > 0)
		{
			$result = $query->result();
			foreach($result as $row)
			{
				echo $row->DESA_NAMA.'|'.$row->ID."\n";
			}
		}			
	}	
}
