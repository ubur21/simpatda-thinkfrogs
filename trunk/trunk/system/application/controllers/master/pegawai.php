...
{	
	...
	{		
		if(b_logged()){
			...
			$row = $this->jabatan->get_all_data();
			for($i=0; $i<count($row); $i++) $jabatan.="'".$row[$i]->ID."':'".$row[$i]->NAMA."',";
			$row = $this->pangkat->get_all_data();
			for($i=0; $i<count($row); $i++) $pangkat.="'".$row[$i]->ID_PANGKAT."':'".$row[$i]->KODE_PANGKAT."',";
			$data['jabatan'] = $jabatan;
			$data['pangkat'] = $pangkat;
			
			$data['main_content'] = $this->load->view('master/daftar_pegawai','',true);
		}else{
	...
	function daftar()
	{
		...
		{				
			...
			for($i=0; $i<count($result); $i++){
				...
				$response->rows[$i]['id']=$result[$i]['PEGAWAI_ID'];
				// data berikut harus sesuai dengan kolom-kolom yang ingin ditampilkan di view (js)
				$response->rows[$i]['cell']=array($result[$i]['PEGAWAI_ID'],
												$result[$i]['NIP'],
												$result[$i]['GELAR_DEPAN'],
												$result[$i]['NAMA'],
												$result[$i]['GELAR_BELAKANG'],
												$result[$i]['JABATAN_ID'],
												$result[$i]['PANGKAT_ID'],
											);
			}
