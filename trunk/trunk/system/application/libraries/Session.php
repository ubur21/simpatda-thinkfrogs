...
class CI_Session {
	...
	function sess_run()
	{
		 ...
		 *
		 */
		 //added condition to see if sess_length is set to expire at browser close using a 0 config or -1 to never expire until two years
	...
	function sess_read()
	{
		...
		//added condition to see if  sess_length is set to expire at browser close
		if ((($session['last_activity'] + $this->sess_length) < $this->now) && ($this->sess_length != 0))
		{
			$this->sess_destroy();
			return FALSE;
		}
	...
	function sess_write_cookie()
	{
		...
		if (isset($_COOKIE[$this->sess_cookie]))
		{
			$l = $this->userdata('sess_length');
			if ($l) $this->sess_length = $l;
		}
		//added condition to see if  sess_length is set to expire at browser close
		setcookie(
					$this->sess_cookie,
	...
	function sess_gc()
	{
		...
		{
			if ($this->sess_length==0) return true;
