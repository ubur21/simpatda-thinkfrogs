...
switch($postConfig['action']){
	case $crudConfig['read']:
		...
		if ($postConfig['q'] <> q) {
			  ...
			  $sql = 'select count(*) as numRows from kelurahan ';
        }
        else {
		...
		if ($postConfig['q'] <> q) {
			...
			$sql = 'select a.lurah_id, a.lurah_kode, a.lurah_nama, a.lurah_kecamatan from kelurahan a join kecamatan b on a.lurah_kecamatan=b.camat_id ';
        } 
        else  {
            //$sql = 'select FIRST '.$intLimit.' SKIP '.$intStart.' '.implode(',',$crudColumns).' from '.$crudTableName;
			$sql = 'select a.lurah_id, a.lurah_kode, a.lurah_nama, a.lurah_kecamatan as lurah_kecamatan from kelurahan a join kecamatan b on a.lurah_kecamatan=b.camat_id ';
