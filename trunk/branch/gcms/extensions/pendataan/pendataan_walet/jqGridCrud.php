...
switch($postConfig['action']){
	case $crudConfig['read']:
		...
		if( $requesttable == 'walet') {
			//    $sql = 'select b.pwalet_id,
			//	    c.kode_Rekening || \' = \' || c.nama_rekening as rekening,
			//	    b.jumlah, b.dasar_tarif, b.dasar_pengenaan, b.persen_tarif, b.nominal'.
			//           ' from pendataan_spt a '.
			//	   ' join pendataan_pwalet b on b.pendataan_id = a.pendataan_id'.
			//	   ' join rekening_kode c on c.id=b.id_rekening';
			$sql = 'select null as pwalet_id,
				    null as rekening,
				    null as jumlah, null as dasar_tarif, null as dasar_pengenaan, null as persen_tarif, null as nominal'.
			           ' from pendataan_spt a '.
				   ' join pendataan_pwalet b on b.pendataan_id = a.pendataan_id'.
				   ' join rekening_kode c on c.id=b.id_rekening';	
		...
		while($row = gcms_fetch_row($result)) { 
			...
			//$data1[2] = $row[11].$row[12].$row[3].$row[9].$row[10];
			$o->rows[$i][$crudConfig['row']]= $data1;
			//$o->rows[$i][$crudConfig['wp_wr_no_urut']]='222222';
			$i++;
			//echo $row;  
			//$o->rows[$i]['idx']=$i;
		}
