...
if( $_GET['edit'] && $_GET['edit'] == 'umum' ) {
	...
	$sql='select a.nominal as totalpajak'.
	     ' from PENDATAAN_SPT a'. 
	     ' join pendataan_pwalet b on b.PENDATAAN_ID = a.PENDATAAN_ID'.
...
elseif($_REQUEST['sender']=='penetapan_PajakWalet') {
	$crudColumns =  array(
		//'no'=>''
		'id'=>'pendataan_id'
		,'pendataan_no'=>2
	...
	$crudTableJoin = 'PENDATAAN_PWALET';
	$ID ="PENDATAAN_";
	$crudTableJoin1 = "PENDAFTARAN";
	
	$crudIdJoin1 = "pendaftaran_id";
	
	include $expath.'jqGridCrudPendataanWalet.php';
}
elseif($_REQUEST['sender']=='DataWalet') {
	switch($_REQUEST['action']) {
		case 'DetailWaletList' :
			$o=null;
			$sql='select \''.quote_smart($_REQUEST['val']).'\' as id,'.
				...
				' join pendataan_pwalet b on b.PENDATAAN_ID = a.PENDATAAN_ID'.
				' join REKENING_KODE c on c.ID = b.ID_REKENING'.
				' where a.pendataan_id='.quote_smart($_REQUEST['val']).' order by b.pwalet_id';
			$o->page=1;
			$o->total=1;
			$o->records=1;
	...
	switch($_REQUEST['action']) {
		...
		case 'get_list':
			...
			if($_REQUEST['cari']=='total') {
				...
				$sql='select b.pwalet_id as id,'.
				'c.kode_rekening || \' - \' || c.NAMA_REKENING as rekening,'.
				'b.DASAR_TARIF as tarif,b.JUMLAH as jumlah,'.
				'b.DASAR_PENGENAAN as pengenaan,b.PERSEN_TARIF as persen,'.
				'b.nominal as pajak'.
				' from PENDATAAN_SPT a'. 
				' join pendataan_pwalet b on b.PENDATAAN_ID = a.PENDATAAN_ID'.
				' join REKENING_KODE c on c.ID = b.ID_REKENING'.
				' where a.pendataan_id='.quote_smart($_REQUEST['id']).' order by b.pwalet_id';
	...
	if($_GET['oper']=='edit') {
		if($_POST['oper']=='del') { //store deleted id
			//unset($_SESSION['delete_box']['pwalet_id']);
			$arr_id = explode(',',$_POST['id']);//
			foreach( $arr_id as $key => $val ) {
				$_SESSION['delete_box']['pwalet_id'][]=$val;
			}
		}
	}
}
