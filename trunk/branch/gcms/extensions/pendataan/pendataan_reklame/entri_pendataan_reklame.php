...
$().ready(function(){
	...
	jQuery("#htmlTable4").jqGrid(
	{
        ...
        ondblClickRow: function(id){ 
            if(id && id!==lastsel2){ 
				jQuery("#htmlTable4").restoreRow(lastsel2); 
        ...
        gridComplete: function(){
			jQuery("#htmlTable4").setGridWidth(450);
            return true;
        }
    }).navGrid('#htmlPager4'
        ,{add:false,edit:false,del:false}
	...
	jQuery("#htmlTable2").jqGrid(
	{
		 ...
		 datatype: 'json',
		 mtype: 'POST',
		 colNames:['id','panjang','lebar','Kode Rekening','Luas','Muka','Jumlah','NJOPR','NSPR','NSR','Pajak'],
		 colModel :[
			{ name:'id' ,index:'id',search:false},
			{ name:'panjang',index:'panjang',search:false,hidden:true},
			{ name:'lebar' ,index:'lebar',search:false,hidden:true},
			...
			{ name:'luas',index:'luas',width:120,editable:true,edittype:'text',editoptions: {size:50,maxlength:50},editrules:{required:true}},
			{ name:'muka',index:'muka',width:80,editable:true,edittype:'text',editrules:{required:true,integer:true},
			  editoptions: {size:20,maxlength:10,dataEvents:[{ type: 'change', fn:function(e){hitungPajak();}}]
			  },
			},
			{ name:'jumlah',index:'jumlah',width:80,editable:true,edittype:'text',align:'right',editrules:{required:true,number:true},formatter:'currency',formatoptions:{decimalSeparator:".",thousandsSeparator:",",decimalPlaces:2},
			  editoptions: {size:20,maxlength:10,dataEvents:[{ type: 'change', fn:function(e){hitungPajak();}}]}
			},
			{ name:'njopr',index:'njopr',width:120,editable:true,edittype:'text',align:'right',editrules: {required:true,number:true},formatter:'currency',formatoptions:{decimalSeparator:".",thousandsSeparator:",",decimalPlaces:2},
			  editoptions: {size:20,maxlength:10,dataEvents:[{ type: 'change', fn:function(e){hitungPajak();}}]}
			},
			{ name:'nspr',index:'nspr',width:80,editable:true,edittype:'text',align:'center',editrules: {required:true,number:true},
			  editoptions:{size:10,maxlength:3,dataEvents:[{ type: 'change', fn:function(e){hitungPajak();}}]}
			},
			{ name:'nsr',index:'nsr',width:80,editable:true,edittype:'text',align:'center',editrules: {required:true,number:true},
			  editoptions:{size:10,maxlength:3,dataEvents:[{ type: 'change', fn:function(e){hitungPajak();}}]}
			},			
        ...
        gridComplete: function(){
			jQuery("#htmlTable2").setGridWidth( document.width - 500 < 100 ? 300 :document.width - 300);
            return true;
        }
    }).navGrid(
		'#htmlPager2',
		{ add:false,edit:true,del:true},
	...
	$("#t_htmlTable2").append("<input type='button' class='btn' id='ver1' value='Versi 1' style='height:20px;font-size:-3'/><input type='button' class='btn' id='ver2' value='Versi 2' style='height:20px;font-size:-3'/><input type='button' class='btn' id='ver3' value='Versi 3' style='height:20px;font-size:-3'/>"); 
	
	$("input#ver1","#t_htmlTable2").click(function(){ 
		$('#form_ver1').dialog('open');
		jQuery.ajax({
			url:'request.php?page=<?=$_REQUEST['page']?>&sender=set_form_reklame&version=1',
			data:'var1=99',
			beforeSend: function(){
				jQuery('#form1').html('<div align="center"><img src="./images/loading.gif"  border="0"/></div>');
			},
			success: function(msg){
				if (msg.length > 0){
	...
	$("input#ver2","#t_htmlTable2").click(function(){ 
		$('#form_ver2').dialog('open');
		jQuery.ajax({
			url:'request.php?page=<?=$_REQUEST['page']?>&sender=set_form_reklame&version=2',
			data:'var1=99',
			beforeSend: function(){
				jQuery('#form2').html('<div align="center"><img src="./images/loading.gif"  border="0"/></div>');
			},
			success: function(msg){
				if (msg.length > 0){
					jQuery('#form2').html(msg);
				}
			}
		});
	});
	
	$("input#ver3","#t_htmlTable2").click(function(){ 
		$('#form_ver3').dialog('open');
		jQuery.ajax({
			url:'request.php?page=<?=$_REQUEST['page']?>&sender=set_form_reklame&version=3',
			data:'var1=99',
			beforeSend: function(){
				jQuery('#form3').html('<div align="center"><img src="./images/loading.gif"  border="0"/></div>');
			},
			success: function(msg){
				if (msg.length > 0){
					jQuery('#form3').html(msg);
				}
			}
		});
	});	
	...
	jQuery("#htmlTable3").jqGrid({
		...
		subGridRowExpanded: function(subgrid_id, row_id) {
			...
			jQuery("#"+subgrid_table_id).jqGrid({ 
				url:"request.php?mod=pendataan_reklame&func=list&sender=default&action=get_list&id="+row_id, 
				datatype: "json", 
				rownumbers: true,
		...
		gridComplete: function(){
			var ids = jQuery("#htmlTable3").getDataIDs();
			for(var i=0;i < ids.length;i++){ 
				...
				jQuery("#htmlTable3").setRowData(ids[i],{act:be+se+ce});
			}			
	...
	$("#t_htmlTable3").append("<input type='button' class='btn' value='Duplikasi' style='height:20px;font-size:-3'/>"); 
	$("input","#t_htmlTable3").click(function(){ 
		alert("Hi! I'm added button at this toolbar"); 
	}); 
			
	jQuery('#proses').click(function(){	
		objForm = document.getElementById('entri_reklame');
				...
				success: function(response){
					...
					if(response!='!'){
						...
						jQuery("#htmlTable2 > tbody > tr").each(function (){
							...
							tmp[this.id].sender='entri_reklame';
	...
	$("#form_ver1").dialog({
		bgiframe: true, resizable: false, height:550, width:900, modal: true, autoOpen: false,
		buttons: {
			'Tutup': function() { $(this).dialog('close');},
			'Tambah':function(){
				var rows = jQuery('#htmlTable2').getGridParam('records');
				var datarow = {id:rows+1,panjang:"22",lebar:"33"};
				var su=jQuery("#htmlTable2").addRowData(1,datarow);
			}
		}
	});	
	...
	jQuery('#coba1').click(function(){
		//var su=jQuery("#htmlTable2").setRowData(1,{rekening:"10",lokasi:"Parung"}); 
		tmp = {};    
		jQuery("#htmlTable2 > tbody > tr").each(function (){
			tmp[this.id] = jQuery("#htmlTable2").getRowData(this.id);
			tmp[this.id].pk=99;
...
<div class='demo'>
	...
	<div id="dialog2" title="Kode Rekening">
		<div style='padding:5px;'>
			<div>
				<div style='padding:5px'>
	...
	<div id="form_ver1" title="Reklame Versi 1">
		<div id='form1'></div>
	</div>	
	
	<div id="form_ver2" title="Reklame Versi 2">
		<div id='form2'></div>
	</div>
	
	<div id="form_ver3" title="Reklame Versi 3">
		<div id='form3'></div>
	</div>
	...
	<div id="tabs">
		...
		<div id="tabs-1">
			<div style='padding:5px;'>
				<fieldset>
					<legend>Isian SPTPD Pajak Reklame</legend>
					<div>
						<div class="form_master">
							<fieldset class="form_frame">
								...
								<div><label>Memo</label><textarea name='memo' id='memo' col="6" row="3"></textarea></label></div>
