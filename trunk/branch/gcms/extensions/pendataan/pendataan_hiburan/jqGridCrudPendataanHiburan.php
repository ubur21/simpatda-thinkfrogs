	...
	case $crudConfig['read']:
		...
		if ($postConfig['q'] <> q) {
		  $sql = 'select FIRST '.$intLimit.' SKIP '.$intStart.' '.implode(',',$crudColumns).' from '.$crudTableName.' where '.$crudFK.' = '.$postConfig[$crudFK];
			...
			$sql = "select 	pendataan_spt.pendataan_id,
							...
							pendataan_spt.nominal													
							
					from pendataan_spt pendataan_spt 
					left join pendaftaran pd on pendataan_spt.pendaftaran_id = pd.pendaftaran_id
					left join pemohon ph on pd.id_pemohon = ph.pemohon_id
						";
            };
        ...
        //if(!empty($crudTableJoin1)){
			//$sql .= ' GROUP BY pendataan_spt.pendataan_id'; 
