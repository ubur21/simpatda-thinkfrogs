...
if(!empty($_GET['Npwp'])){
	...
	if(!empty($row->pendataan_id)){
		$count = b_fetch("select count(*) from pendataan_hiburan_detail where pendataan_id ='".$row->pendataan_id."'");
		if($count){
			?>
	...
	*/		//alert(<?=$count?>);
			<?
			$cari_detail_hiburan = "select * from pendataan_hiburan_detail where pendataan_id ='".$row->pendataan_id."'";
			$query_detail_hiburan = gcms_query($cari_detail_hiburan);
			$i=1;
			$total = 0;
			while($rows = gcms_fetch_object($query_detail_hiburan)){
			//$total += $rows->jumlah_kamar*$rows->tarif_kamar;
			?>
	//		document.getElementById('DetailHid'+<?=$i?>).value='<?=$rows->hiburan_detail_id?>';
...
if(!empty($_GET['RekeningHiburan'])){
	$cr="select * from rekening_kode where id='".$_GET['RekeningHiburan']."'";
	$xx=gcms_query($cr);
	$row=gcms_fetch_object($xx);
...
if( $_GET['rekening'] ) {
	...
	*/
	...
	//$str='<select id="kode_rekening" name="kode_rekening" >';
	$str ='';
	...
	while( $rows = gcms_fetch_object( $result ) ) {
		...
		$str.='<option value="'.$rows->id.'">'.$rows->kode_rekening." - ".$nama_rek.'</option>';
	}
	//for ($a=0;$a<2;$a++){
	//$str.='<option >abc'.$a.'</option>';
	//}
	//$str.='</select>';
	?>
	var kodeRek='<?=$str?>';
	//document.getElementById('kode_rekening1').innerHtml ='<?=$str?>';
	<?php
}
if(!empty($_GET['DataForm'])){
	$idPendataan = b_fetch("select pendataan_id from pendataan_hiburan where hiburan_id ='".$_GET['DataForm']."'");
	$count = b_fetch("select count(*) from pendataan_hiburan_detail where pendataan_id ='".$idPendataan."'");
		if($count){
			?>
	/*		$("tr.isi_detail").remove();
			for(id=1; id <= <?=$count?>; id++){
	...
	*/	<? 
			...
			$cari_master ="select * from pendataan_hiburan pr
							left join pendataan_spt ps on pr.pendataan_id = ps.pendataan_id
							left join pendataan_hiburan_detail pd on ps.pendataan_id = pd.pendataan_id
							...
							left join kecamatan kc on k.lurah_kecamatan = kc.camat_id
							where pr.hiburan_id ='".$_GET['DataForm']."'";
			...
			while($row = gcms_fetch_object($query_master)){
				...
				$nama = $row->hiburan_nama;
				$alamat = showMultiLine($row->hiburan_alamat);
				...
				?>
		//		document.getElementById('DetailHid'+<?=$i?>).value='<?=$row->hiburan_detail_id?>';
