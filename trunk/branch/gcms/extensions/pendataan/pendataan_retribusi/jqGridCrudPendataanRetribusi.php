	...
	case $crudConfig['read']:
		...
		if ($postConfig['q'] <> q) {
		      ...
		      $sql='select count(*) as numRows from '.$crudTableName;
			   ...
			   $sql='select count('.$postConfig['id'].') as numRows from pendataan_spt pendataan_spt
					left join pendaftaran pendaftaran on pendataan_spt.pendaftaran_id = pendaftaran.pendaftaran_id
					left join pemohon pemohon on pemohon.pemohon_id = pendaftaran.id_pemohon
					left join kelurahan kelurahan on kelurahan.lurah_id = pemohon.id_desa
					left join kecamatan kecamatan on kecamatan.camat_id = kelurahan.lurah_kecamatan
					where pendataan_spt.jenis_pendataan = \'RETRIBUSI\'';
			   }; 
		...
		if ($postConfig['q'] <> q) {
		  $sql = 'select FIRST '.$intLimit.' SKIP '.$intStart.' '.implode(',',$crudColumns).' from '.$crudTableName.' where '.$crudFK.' = '.$postConfig[$crudFK];
            ...
            //$sql = 'select FIRST '.$intLimit.' SKIP '.$intStart.' '.implode(',',$crudColumns).' from '.$crudTableName;
		    		   $sql = 'select FIRST '.$intLimit.' SKIP '.$intStart.'
						   pendataan_spt.pendataan_id as id,
						   pendataan_spt.tgl_entry,
						   pendaftaran.npwp,
						   pemohon.nama as nama_wpwr,
						   pemohon.alamat || \',\' || kelurahan.lurah_nama || \',\' || kecamatan.camat_nama as alamat,
						   pendataan_spt.jenis_pungutan,
						   pendataan_spt.periode_awal,
						   pendataan_spt.periode_akhir
					from pendataan_spt pendataan_spt
					left join pendaftaran pendaftaran on pendataan_spt.pendaftaran_id = pendaftaran.pendaftaran_id
					left join pemohon pemohon on pemohon.pemohon_id = pendaftaran.id_pemohon
					left join kelurahan kelurahan on kelurahan.lurah_id = pemohon.id_desa
					left join kecamatan kecamatan on kecamatan.camat_id = kelurahan.lurah_kecamatan
...
//		print_r( $crudColumnValues );
	...
	//echo "delete";
		...
		$crudColumnValues['id']=explode(",",$crudColumnValues['id']);
		$crudColumnValues['id']=implode("','",$crudColumnValues['id']);
		
		if(isset($crudTableJoin)) {
			$sql = 'Delete from '.$crudTableJoin.' where '.$postConfig['id'].' in('.$crudColumnValues['id'].');';	
			if($DEBUGMODE == 1){$firephp->info($sql,'query');}
			gcms_query( $sql ) 
			or die($firephp->error('Couldn t execute query.'.mysql_error()));
		}
            
		$sql = 'Delete from '.$crudTableName.' where '.$postConfig['id'].' in('.$crudColumnValues['id'].');';
