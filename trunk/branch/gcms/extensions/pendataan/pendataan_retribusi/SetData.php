...
if( $_GET['walet'] ) {
	$query = "SELECT a.id, a.kode_rekening, a.nama_rekening,  a.persen_tarif, a.tarif_dasar from rekening_kode a";
	...
	while( $rows = gcms_fetch_object( $result ) ) {
		...
		?>
		{
		  ...
		  "tdasar"  : "<?php echo $rows->tarif_dasar;?>"
		},
		<?php
	}
	?>
...
if( $_GET['persentarif'] ) {
	$sql = "SELECT a.PERSEN_TARIF,a.TARIF_DASAR from rekening_kode a where a.ID = ".$_GET['persentarif'];
	$result = gcms_query( $sql );
	while( $rows = gcms_fetch_object( $result ) ) {
		$persentarif = $rows->persen_tarif;
		$tarifdasar = $rows->tarif_dasar;
	}
	
	
	echo "[{ 'tarif' : '".$persentarif."', 'tdasar' : '".$tarifdasar."' }]";
}
if( $_GET['setnomor'] ) {
	$query = "select max(pendataan_no) from pendataan_spt where jenis_pendataan = 'WALET';";
	$result = gcms_query( $query );
	$str = "[";
	while( $rows = gcms_fetch_object( $result ) ) {
		$nomor = $rows->max;
...
//	$sql = "select * from pendataan_pwalet p
//		where p.pendataan_id = ".$_REQUEST['id'];
//	
//	$result = gcms_query( $sql );
//	
...
//}
if( $_REQUEST['detail'] ) {
	$arr = array ('page'=>1,'total'=>0,'records'=>0,
		      'rows'=>array(
					array('id'=>0,'cell'=>array('sortname',$_REQUEST['sidx'])),
					array('id'=>1,'cell'=>array('page',$_REQUEST['page'])),
					array('id'=>2,'cell'=>array('rowNum',$_REQUEST['sord']))
				   )
		      );
