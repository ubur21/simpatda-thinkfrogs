...
include "entri.php";
yuiBeginEntry("entri_pendataan_PajakRestoran");
...
jQuery(document).ready(function(){ 
  jQuery("#htmlTable").jqGrid({
    url:'request.php?page=<?=$_REQUEST['page']?>&sender=penetapan_PajakRestoran',
	editurl:'request.php?page=<?=$_REQUEST['page']?>&sender=penetapan_PajakRestoran&oper=edit',
    datatype: 'json',
    mtype: 'POST',
    colNames:['id','Tanggal Entri','NPWPD/NPWRD','Nama WP/WR', 'Alamat','Sistem Pemungutan','Periode Penjualan Awal', 'Periode Penjualan Akhir','Nama Rekening','Dasar Pengenaan','Persen (%)','Pajak'],
    colModel :[{
		name:'id'
		,index:'id'
		,width:20
		...
		name:'TglEntri',index:'TglEntri',width:180,editable:false,edittype:'text',editoptions: {size:50, maxlength: 50},editrules: {required:true}
	},{
		...
		name:'TarifDasar',index:'TarifDasar',width:150, align:"right",sorttype:"float",editable:false,edittype:'text',editoptions: {size:50, maxlength: 50},editrules: {required:true}
	},{
		name:'Persen',index:'Persen',width:80,editable:false, align:"right",sorttype:"float",edittype:'text',editoptions: {size:50, maxlength: 50},editrules: {required:true}
	},{
    ...
    height:150,
    rowNum:5,
    rowList:[5,10,15,25,50,100],
    shrinkToFit:false,
    sortname: 'id',
    sortorder: 'asc',
    viewrecords: true,
    caption: 'Data Penetapan Pajak Restoran',
	...
	subGridRowExpanded: function(subgrid_id, id) {
		...
		jQuery("#"+subgrid_table_id).jqGrid({ 
			url:"request.php?mod=pendataan_restoran&func=list&sender=DataRestoran&action=DetailRestoranList&val="+id, 
			datatype: "json", 
			rownumbers: true,
			colNames: ['id','Jumlah Meja','Jumlah Kursi','Jumlah Tamu /hari'], 
			colModel: [
				{ name:'id' ,index:'id'	,width:20,key:true ,search:false},
				{name:"jumlah_meja",index:"jumlah_meja",width:120,search:false,align:"left"}, 
				{name:"jumlah_kursi",index:"jumlah_kursi",width:70,search:false}, 
				{name:"jumlah_tamu_per_hari",index:"jumlah_tamu_per_hari",width:120,align:"center"},
...
//End Sub
		...
		ondblClickRow: function(id){ 
            if(id && id!==lastsel){
				jQuery("#htmlTable5").setGridParam({url:"request.php?page=<?=$_REQUEST['page']?>&sender=DataRestoran&action=DetailRestoran&val="+id}).trigger("reloadGrid");
				jQuery.post("request.php?page=<?=$_REQUEST['page']?>&sender=get_DataFormRestoran&val="+id, {},
					function(data){						
				...
				jQuery("#htmlTable5").setGridParam({url:'request.php?page=<?=$_REQUEST['page']?>&sender=DataHotel&action=DetailHotel&val='+id});
                //lastsel=id; 
            }
        },
...
<div class='demo'>
	<div id="tabs">
		<ul>
			<li><a href="#tabs-1"><em>Form</em></a></li>
				...
				<legend>PENETAPAN PAJAK RESTORAN</legend>
					<div id='asb_simulasi_form'>
						<div class="singleSide">
							...
							<label class="leftField"><span>Periode Penjualan <b class="wajib">*</b></span>
			...
			<div style='padding:5px;'>
				<fieldset>
					...
					<div id='asb_simulasi_form'>
						...
						<!--<table id="detail">
						<tr><th>Jumlah Meja</th><th>Jumlah Kursi</th><th>Jumlah Tamu /Hari</th>
...
jQuery('#proses').click(function(){	
	formObject = document.getElementById('entri_pendataan_PajakRestoran');
					...
					//	alert('Response'+response);
						if(rows>0){
							jQuery("#htmlTable5 > tbody > tr").each(function (){
								...
								tmp[this.id].sender='entri_pendataan_PajakRestoran';
...
var callback = {
	success: function(o) {
	var formObject = document.getElementById("entri_pendataan_PajakRestoran");
