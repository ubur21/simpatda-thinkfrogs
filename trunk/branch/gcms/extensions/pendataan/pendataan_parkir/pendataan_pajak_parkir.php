	...
	if(saveEntry(objForm)) {
		...
		success: function(response) {
			...
			jQuery("#gridDetail > tbody > tr").each(function () {
			    ...
			    tmp[this.id].sender='entri_penetapan_PajakParkir';
...
function ResetStatus() {
    ...
    $('#caction').val('');
    $('#idmasters').val('');
    var objForm = document.getElementById('entri_penetapan_PajakParkir');
    jQuery("#gridDetail").setGridParam({url:'request.php?page=<?=$_REQUEST['page']?>&sender=default'}).trigger("reloadGrid");
    objForm.reset();
...
jQuery(document).ready(function(){ 
  jQuery("#htmlTable").jqGrid({
    ...
    sortorder: 'asc',
    viewrecords: true,
    caption: 'Data Pendataan Pajak Parkir',
		subGrid:true,
		multiselect:true,
		multiboxonly:true,
		toolbar:[true,'top'],
		...
		subGridRowExpanded:function(subgrid_id,id) {
		    ...
		    jQuery("#"+subgrid_table_id).jqGrid({
			    url:"request.php?mod=pendataan_parkir&func=list&sender=DataParkir&action=DetailParkirList&val="+id,
			    datatype:"json",
			    rownumbers:true,
...
<div class="demo">
    <div id="tabs">
		...
		<!--CONTENT-->
			...
			<input type="hidden" name="jenis_pendataan" id="jenis_pendataan" value="PARKIR" />
		    </div>
