...
if( $_GET['edit'] && $_GET['edit'] == 'umum' ) {
	...
	$sql='select a.nominal as totalpajak'.
	     ...
	     ' join pendataan_pparkir b on b.PENDATAAN_ID = a.PENDATAAN_ID'.
...
elseif($_REQUEST['sender']=='penetapan_PajakParkir') {
	$crudColumns =  array(
		'no'=>''
		,'id'=>'pendataan_id'
		,'pendataan_no'=>2
	...
	$crudTableJoin = 'PENDATAAN_PPARKIR';
	$ID ="PENDATAAN_";
	$crudTableJoin1 = "PENDAFTARAN";
	
	$crudIdJoin1 = "pendaftaran_id";
	
	include $expath.'jqGridCrudPendataanParkir.php';
}
elseif( $_REQUEST['sender']=='DataParkir' ) {
	switch($_REQUEST['action']) {
		case 'DetailParkirList' :
			$o=null;
			$sql='select \''.$_REQUEST['val'].'\' as id,'.
				...
				' join pendataan_pparkir b on b.PENDATAAN_ID = a.PENDATAAN_ID'.
				' join REKENING_KODE c on c.ID = b.ID_REKENING'.
				' where a.pendataan_id='.$_REQUEST['val'].' order by b.pparkir_id';
			$o->page=1;
			$o->total=1;
			$o->records=1;
			$i=0;$pajak=0;
	...
	switch($_REQUEST['action']){
		...
		case 'get_list':
			...
			if($_REQUEST['cari']=='total') {
				...
				' join pendataan_pparkir b on b.PENDATAAN_ID = a.PENDATAAN_ID'.
				' join REKENING_KODE c on c.ID = b.ID_REKENING'.
				' where a.pendataan_id='.quote_smart($_REQUEST['id']).' order by b.pparkir_id';
	...
	if($_GET['oper']=='edit') {
		if($_POST['oper']=='del') { //store deleted id
			//unset($_SESSION['delete_box']['pparkir_id']);
			$arr_id = explode(',',$_POST['id']);//
			foreach( $arr_id as $key => $val ) {
				$_SESSION['delete_box']['pparkir_id'][]=$val;
			}
		}
	}
}
