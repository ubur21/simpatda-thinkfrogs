...
if($_REQUEST['sender']=="entri_pajak_golc"){
	...
	if($_REQUEST['action']=='edit'){
		...
		if(isset($_REQUEST['detail'])){
			...
			$exception = array('galianc_id','pendataan_id','lokasi');
			...
			$str_where = ' where galianc_id='.quote_smart($_REQUEST['id']);
			ibase_trans();
			$a = $fbird->FBUpdate('pendataan_galianc',$other_request,$exception,$str_where); unset($exception); unset($other_request);
			...
			if($_REQUEST['spt']==''){
				$exception = array('spt_id','pendataan_id','jenis_pendataan');
				$other_request = array('pendataan_no'=>(int)$_REQUEST['nomor_reg']);
			}else{
				$exception = array('pendataan_id','jenis_pendataan');
		...
		if(isset($_REQUEST['detail'])){
			...
			$id_rekening=getIdRekening($_REQUEST['rekening']);	
			$exception = array('galianc_id','lokasi');
			$other_request = array(
			...
			$a = $fbird->FBInsert('pendataan_galianc',$other_request,$exception);unset($exception); unset($other_request);
			...
			if($_REQUEST['spt']==''){
				$exception = array('spt_id');
				$other_request = array('pendataan_id'=>$ID_HEADER,'pendataan_no'=>$no_spt,'jenis_pendataan'=>'GALIAN C');
			}else{
				$exception = array();
				$other_request = array('pendataan_id'=>$ID_HEADER,'pendataan_no'=>$no_spt,'jenis_pendataan'=>'GALIAN C','spt_id'=>$_REQUEST['spt']);
			}
