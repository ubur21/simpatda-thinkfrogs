...
switch($postConfig['action']){
	case $crudConfig['read']:
		...
		if ($postConfig['q'] <> q) {
			...
			$sql = 
			   'SELECT
				...
				WHERE c.objek_pdrd=\'PRIBADI\' and a.jenis_pendataan=\'GALIAN C\'
				union
				SELECT
				  a.pendataan_id as id,
				  a.pendataan_no,
				...
				WHERE c.objek_pdrd=\'BU\' and a.jenis_pendataan=\'GALIAN C\' ';
		...
		$o->userdata['jenis_pungutan'] = 'Total'; 
		$o->userdata['nominal_pajak'] = $nominal;
		break;
...
//		print_r( $crudColumnValues );
	...
	case $crudConfig['delete']:
		...
		foreach($id_del as $del){
			...
			if($tblContent!='') $del_content = 'delete from '.$tblContent.' where '.$crudFK.' = '.quote_smart($del);
			if($tblDetail!='')  $del_detail  = 'delete from '.$tblDetail.' where '.$crudFK.' = '.quote_smart($del);
			if($tblHeader!='')  $del_header  = 'delete from '.$tblHeader.' where '.$crudFK.' = '.quote_smart($del);		
		
			if(isset($del_content)) gcms_query($del_content) or die($firephp->error('Couldn t execute query.'.mysql_error()));
			if(isset($del_detail)) gcms_query($del_detail) or die($firephp->error('Couldn t execute query.'.mysql_error()));
			if(isset($del_header)) gcms_query($del_header) or die($firephp->error('Couldn t execute query.'.mysql_error()));		
		}		
		break;
	}
