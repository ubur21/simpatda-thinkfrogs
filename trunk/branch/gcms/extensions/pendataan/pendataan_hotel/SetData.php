...
if(!empty($_GET['Npwp'])){
	...
	if(!empty($row->pendataan_id)){
		$count = b_fetch("select count(*) from pendataan_hotel_detail where pendataan_id ='".$row->pendataan_id."'");
		if($count){
			?>
			...
			for(id=1; id <= <?=$count?>; id++){
				...
				var str = " <tr id='" + id + "' "
						 +" class='isi_detail'> ";			
				//var data = kodeRek;
				str += "<td><input type='hidden' id=\"DetailHid"+id+"\" name=\"DetailHid"+id+"\" ><input  id=\"nama_kamar"+id+"\" type='text' name=\"nama_kamar"+id+"\" value='' size='30'></td><td><input class='field_angka' id=\"jumlah_kamar"+id+"\" type='text' name=\"jumlah_kamar"+id+"\" size='10' onblur=\"hitung_tarif("+id+");\" onkeyup=\"hitung_tarif("+id+");\" ></td><td><input class='field_angka' id=\"tarif_kamar"+id+"\" type='text' name=\"tarif_kamar"+id+"\" size='20' onblur=\"hitung_tarif("+id+");\" /></td>";
			...
			//alert(<?=$count?>);
			<?
			$cari_detail_hotel = "select * from pendataan_hotel_detail where pendataan_id ='".$row->pendataan_id."'";
			$query_detail_hotel = gcms_query($cari_detail_hotel);
			$i=1;
			$total = 0;
			while($rows = gcms_fetch_object($query_detail_hotel)){
			$total += $rows->jumlah_kamar*$rows->tarif_kamar;
			?>
			//document.getElementById('DetailHid'+<?=$i?>).value='<?=$rows->hotel_detail_id?>';
			//document.getElementById('nama_kamar'+<?=$i?>).value='<?=$rows->nama_kamar?>';
			//document.getElementById('jumlah_kamar'+<?=$i?>).value=formatCurrency('<?=$rows->jumlah_kamar?>');
			//document.getElementById('tarif_kamar'+<?=$i?>).value=formatCurrency('<?=$rows->tarif_kamar?>');
			<?
			$i++;
			}
			?>
...
if(!empty($_GET['RekeningHotel'])){
	$cr="select * from rekening_kode where id='".$_GET['RekeningHotel']."'";
	$xx=gcms_query($cr);
	$row=gcms_fetch_object($xx);
...
if(!empty($_GET['DataForm'])){
	$idPendataan = b_fetch("select pendataan_id from pendataan_hotel where hotel_id ='".$_GET['DataForm']."'");
	$count = b_fetch("select count(*) from pendataan_hotel_detail where pendataan_id ='".$idPendataan."'");
		if($count){
			?>
			/*$("tr.isi_detail").remove();
			//reset hidden id. to 0//
		...
		<? 
		//}
			$cari_master ="select * from pendataan_hotel ph
							...
							left join kecamatan kc on k.lurah_kecamatan = kc.camat_id
							where ph.hotel_id ='".$_GET['DataForm']."'";
			...
			while($row = gcms_fetch_object($query_master)){
				...
				$nama = $row->hotel_nama;
				$alamat = showMultiLine($row->hotel_alamat);
				...
				$TarifDasar = $row->nominal;
				$Persen = $row->persen_tarif;
				$Pajak = ($row->nominal*$row->persen_tarif)/100;
				?>
				//document.getElementById('DetailHid'+<?=$i?>).value='<?=$row->hotel_detail_id?>';
				//document.getElementById('nama_kamar'+<?=$i?>).value='<?=$row->nama_kamar?>';
				//document.getElementById('jumlah_kamar'+<?=$i?>).value=formatCurrency('<?=$row->jumlah_kamar?>');
				//document.getElementById('tarif_kamar'+<?=$i?>).value=formatCurrency('<?=$row->tarif_kamar?>');
				<?
				$i++;
			}
		}else{
