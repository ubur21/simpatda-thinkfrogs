	...
	case $crudConfig['read']:
		...
		if ($postConfig['q'] <> q) {
			...
			$sql = 'select
					...
					join penetapan_pr b on b.PENETAPAN_PR_ID=a.PENETAPAN_PR_ID
					join pendataan_spt c on c.PENDATAAN_ID=a.PENDATAAN_ID
					join REF_JENIS_PAJAK_RETRIBUSI d on d.KODE=c.JENIS_PENDATAAN
					join REF_PEMBAYARAN e on e.KODE=a.ID_REF_PEMBAYARAN
					join skpd f on f.ID=a.SKPD_ID
					where a.JENIS_PUNGUTAN=\'OFFICE\' ';
        }
        else {
		...
		if ($postConfig['q'] <> q) {
			...
			$sql = 'select FIRST '.$intLimit.' SKIP '.$intStart.'
					a.PENERIMAAN_PR_ID as id,
					a.PENERIMAAN_PR_NO,
					a.NOMINAL_PAJAK,
					a.TGL_PENERIMAAN,
					b.NO_PENETAPAN as no_kohir,
					d.JENIS,
					e.KETERANGAN as via_bayar,
					f.NAMA_SKPD
					from penerimaan_pr a
					join penetapan_pr b on b.PENETAPAN_PR_ID=a.PENETAPAN_PR_ID
					join pendataan_spt c on c.PENDATAAN_ID=a.PENDATAAN_ID
