...
$().ready(function(){
	...
	{
		 url:'request.php?mod=penetapan_pr&func=list&sender=pilih_spt',
		 datatype: 'json',
		 mtype: 'POST',
		 colNames:['id','No. Pendataan','Tgl. Pendataan','Nominal Pendataan'],
		 colModel :[
			{ name:'id' ,index:'id',search:false},
			{ name:'kohir',index:'kohir',width:90},
		...
		sortorder: 'asc',
		viewrecords: true,
		caption: 'Daftar No. Pendataan',
        ondblClickRow: function(id){ 
            if(id && id!==lastsel2){ 
				jQuery('#id_kohir').val(id);
                jQuery.ajax({
					url:'request.php?page=<?=$_REQUEST['page']?>&sender=setFormSelf&id='+id
					,dataType:'json'
	...
	jQuery("#htmlTable4").jqGrid({
		url:'request.php?page=<?=$_REQUEST['page']?>&sender=daftar_self',
		editurl:'request.php?page=<?=$_REQUEST['page']?>&sender=daftar_self&oper=edit',
		datatype: 'json',
		mtype: 'POST',
...
function setNomor(tgl){ 
	jQuery.ajax({
		url:'request.php?page=<?=$_REQUEST['page']?>&sender=set_nomor_self&tgl='+tgl,
		dataType:'text',
					...
					<div>
						<div class="form_master">
							<fieldset class="form_frame">
								...
								<div><label>No. Penerimaan</label><input type="text" name="nomor_reg" id='nomor_reg' size='10' value="<?=sprintf('%05d',getNoPenerimaan('SELF'))?>" readonly/></div>
