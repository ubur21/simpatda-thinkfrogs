...
if($_REQUEST['str_id']!=''){
	...
	$tmp = explode('|',$_REQUEST['str_id']); $param='';
...
switch($postConfig['action']){
	case $crudConfig['read']:
		...
		if ($postConfig['q'] <> q) {
			...
			$sql = 'select
					count(*) as numRows
					from penerimaan_pr a
					join penerimaan_pr_content b on a.PENERIMAAN_PR_ID=b.PENERIMAAN_PR_ID
					join rekening_kode c on b.ID_REKENING=c.ID
					where 1=1 '.$cwhere.'
					group by 
					b.ID_REKENING,
					a.THN_PENERIMAAN,
		...
		if($postConfig['search'] == 'true'){
        	$filter .= ' and UPPER('.$postConfig['searchField'].') '.fnSearchCondition($_POST['searchOper'], $postConfig['searchStr']);
		};	
		...
		if ($postConfig['q'] <> q) {
			...
			$sql = 'select FIRST '.$intLimit.' SKIP '.$intStart.' 
					...
					join penerimaan_pr_content b on a.PENERIMAAN_PR_ID=b.PENERIMAAN_PR_ID
					join rekening_kode c on b.ID_REKENING=c.ID
					left join sts_content d on d.ID_REKENING=b.ID_REKENING
					where 1=1 and d.STS_CONTENT_ID is null and a.id_ref_pembayaran=\'BD\' '.$cwhere.'
					group by 
					b.ID_REKENING,
					a.THN_PENERIMAAN,
					c.ID,
					c.KODE_REKENING,
					c.NAMA_REKENING ';
					//left join penyetoran_pr_content d on b.id_rekening=d.id_rekening
					// and d.id_rekening is null
