...
$DEBUGMODE = 0;								/* Set to 1 if you want firephp output */
...
switch($postConfig['action']){
	case $crudConfig['read']:
		...
		if ($postConfig['q'] <> q) {
			...
			$sql = 'select
					count(*) as numRows
					from '.$crudTableName.' a 
					left join penetapan_pr_content b on b.pendataan_id=a.pendataan_id
					where 1=1 and b.pendataan_id is null and a.jenis_pungutan=\'OFFICE\' '.$filter.' '.$search;
        }
        else {
		...
		if($_REQUEST['a']!=''){
			$filter.=' and jenis_pendaftaran='.quote_smart($_REQUEST['a']);
		}
		if($_REQUEST['b']!=''){
			$filter.=' and jenis_pungutan='.quote_smart($_REQUEST['b']);
		}
		
		if($_REQUEST['tgl1']!='' && $_REQUEST['tgl2']!=''){
			$filter.=' and tgl_entry between '.quote_smart(date('Y.m.d',strtotime($_REQUEST['tgl1']))).' and '.quote_smart(date('Y.m.d',strtotime($_REQUEST['tgl2'])));
		}
		
		if($_REQUEST['tgl1']!='' && $_REQUEST['tgl2']=='' ){
			$filter.= ' and tgl_entry='.quote_smart(date('Y.m.d',strtotime($_REQUEST['tgl1'])));
		}
		
		if($_REQUEST['tgl1']=='' && $_REQUEST['tgl2']!='' ){
			$filter.= ' and tgl_entry='.quote_smart(date('Y.m.d',strtotime($_REQUEST['tgl2'])));
		}		
