	...
	case $crudConfig['read']:
		...
		if ($postConfig['q'] <> q) {
			...
			$sql = 'select
					...
					left join penerimaan_pr b on a.pendataan_id=b.pendataan_id
					where b.pendataan_id is null ';
        }
        else {
			$sql = 'select
					count(*) as numRows
		...
		if ($postConfig['q'] <> q) {
            ...
            //$sql = 'select FIRST '.$intLimit.' SKIP '.$intStart.' id, kode_skpd as kode, nama_skpd as nama from '.$crudTableName.' where 1=1 '.$filter;
			$sql = 'select FIRST '.$intLimit.' SKIP '.$intStart.'
					a.pendataan_id,
					a.pendataan_no,
					a.tgl_entry,
