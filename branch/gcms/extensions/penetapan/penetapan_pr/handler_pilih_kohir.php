	...
	case $crudConfig['read']:
		...
		if ($postConfig['q'] <> q) {
			...
			$sql = 'select
					count(*) as numRows
					from PENETAPAN_PR a
					left join PENERIMAAN_PR b on a.PENETAPAN_PR_ID=b.PENETAPAN_PR_ID
					where b.PENETAPAN_PR_ID is null ';
        }
        else {
		...
		if ($postConfig['q'] <> q) {
			...
			$sql = 'select FIRST '.$intLimit.' SKIP '.$intStart.'
					...
					a.NO_PENETAPAN as nomor,
					a.TGL_PENETAPAN as tgl,
					a.TGL_SETOR as setor,
					a.NOMINAL_PENETAPAN as nominal
					from PENETAPAN_PR a
					left join PENERIMAAN_PR b on a.PENETAPAN_PR_ID=b.PENETAPAN_PR_ID
					where b.PENETAPAN_PR_ID is null ';
