...
include "entri.php";
yuiBeginEntry("entri_penetapan_PajakRetribusi");
...
$(document).ready( function() {
	...
	objForm = document.getElementById('entri_penetapan_PajakRetribusi');
	...
	if(saveEntry(objForm)) {
		...
		success: function(response) {
			...
			jQuery("#gridDetail > tbody > tr").each(function () {
			    ...
			    tmp[this.id].sender='entri_penetapan_PajakRetribusi';
	...
	objForm = document.getElementById('entri_penetapan_PajakParkir');
	objForm.reset();
...
var callback = {
    success: function(o) {
		...
		var formObject = document.getElementById("entri_penetapan_PajakRetribusi");
...
function ResetStatus() {
    ...
    $("input[name=pretribusiid[]]").remove();
    ...
    $('#caction').val('');
    $('#idmasters').val('');
      var objForm = document.getElementById('entri_penetapan_PajakRetribusi');
    jQuery("#gridDetail").setGridParam({url:'request.php?page=<?=$_REQUEST['page']?>&sender=default'}).trigger("reloadGrid");
    objForm.reset();
}
...
function processAdd(response, postdata){
		...
		var new_val = old_val+Number(postdata.retribusi);
...
function processEdit(response, postdata){
		...
		total+=Number(postdata.retribusi);
		jQuery('input#nominal').val(formatCurrency(total));
		var success = true; var message = ""; var new_id = "";
		return [success,message,new_id];
}

function processCheck(rowid,rowdata,rowelem) {
    var numRows=0;
    jQuery("#gridDetail > tbody > tr").each(function() {
	numRows+=1;
    });
    if(numRows > 1) {
	deleteRow('gridDetail',rowid);
	alert('Detail Cukup Satu!');
	return false;
    }
    return true;
}
...
function hitungPajak(){
		...
		retribusi  = dasar*0.1; id = jQuery('input#id').val();
		...
		//});
		//total+=Number(pajak);
		jQuery('input#retribusi').val(retribusi); tmp={}; total=0;
		jQuery("#gridDetail > tbody > tr").each(function (){
			tmp = jQuery("#gridDetail").getRowData(this.id);
			if(id!=tmp.id) total+=Number(tmp.retribusi);
		});
		total+=Number(retribusi);
		//jQuery('input#nominal').val(formatCurrency(total));
}
...
jQuery(document).ready(function(){ 
  jQuery("#htmlTable").jqGrid({
    url:'request.php?page=<?=$_REQUEST['page']?>&sender=penetapan_PajakRetribusi',
    editurl:'request.php?page=<?=$_REQUEST['page']?>&sender=penetapan_PajakRetribusi&oper=edit',
    datatype: 'json',
    mtype: 'POST',
    ...
    sortorder: 'asc',
    viewrecords: true,
    caption: 'Data Pendataan Retribusi',
	subGrid:true,
	multiselect:true,
	multiboxonly:true,
	toolbar:[true,'top'],
	...
	subGridRowExpanded:function(subgrid_id,id) {
		...
		url:"request.php?mod=pendataan_retribusi&func=list&sender=DataRetribusi&action=DetailRetribusiList&val="+id,
		datatype:"json",
		rownumbers:true,
		colNames:['id','Kode Rekening','Jumlah','Tarif','Dasar Pengenaan','Retribusi'],
		colModel:[
		  ...
		  { name:'retribusi',index:'retribusi',width:100,align:'right',formatter:'currency',formatoptions:{decimalSeparator:'.',thousandsSeparator:',',decimalPlaces:2}}
		],
...
var lastsel2;
	...
	colNames:['id','Kode Rekening','Jumlah','Tarif Dasar','Dasar Pengenaan','Retribusi'],
	colModel:[
	...
	//    },
	    { 	name:'retribusi',index:'retribusi',width:80,editable:true,edittype:'text',align:'right',
...
<div class="demo">
    <div id="tabs">
		...
		<!--CONTENT-->
			...
			<input type="hidden" name="pretribusiid" id='pretribusiid' />
			<input type="hidden" name="id_desa" id='id_desa' value=""  />
			<input type="hidden" name="jenis_pendataan" id="jenis_pendataan" value="RETRIBUSI" />
		    </div>
			<div style="padding:5px;">
		      <fieldset>
			<legend>Isian SPTPD Retribusi</legend>
			<div class="form_master">
				...
				<div><label>Sistem Pemungutan</label><select title="jenis_pungutan" id="jenis_pungutan" name="jenis_pungutan" size="1"><!--<option value="Pilih" >Pilih</option>--><option value="OFFICIAL" selected>Official Assesment</option><!--<option value="SELF" >Self Assesment</option>--></select></div>
		    ...
		    <div style="padding:5px;">
			<fieldset>
			    <legend>Detail Kegiatan ( max. 1 )</legend>
			    <div class="form_master">
			    <fieldset>
				    <div>
