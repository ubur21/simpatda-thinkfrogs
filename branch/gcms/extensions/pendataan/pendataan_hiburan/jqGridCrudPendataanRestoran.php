	...
	case $crudConfig['read']:
		...
		if ($postConfig['q'] <> q) {
		  $sql = 'select FIRST '.$intLimit.' SKIP '.$intStart.' '.implode(',',$crudColumns).' from '.$crudTableName.' where '.$crudFK.' = '.$postConfig[$crudFK];
			...
			$sql = "select 	pendataan_restoran.restoran_id,
							pendataan_spt.tgl_entry,
							pendaftaran.npwp,
							pendataan_restoran.restoran_nama,
							pendataan_restoran.restoran_alamat,
							pendataan_spt.jenis_pungutan,
							pendataan_spt.periode_awal,
							pendataan_spt.periode_akhir,
							rekening_kode.nama_rekening,
							pendataan_restoran.dasar_pengenaan,
							pendataan_restoran.persen_tarif,
							rekening_kode.kode_rekening
							 
					from pendataan_restoran pendataan_restoran 
						left join pendataan_spt pendataan_spt on pendataan_restoran.pendataan_id = pendataan_spt.pendataan_id
						left join pendaftaran pendaftaran on pendataan_spt.pendaftaran_id = pendaftaran.pendaftaran_id
						left join rekening_kode rekening_kode on pendataan_restoran.id_rekening = rekening_kode.id";
            };
